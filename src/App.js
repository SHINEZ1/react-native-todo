...
const List = styled.ScrollView`
  flex:1;
  width:${({width})=>width - 40}px;   //전체폭에서 40 뺀 수치
  `;
...
export default function App() {
  ...
  const _addTask = () =>{
    console.log('입력완료');
    const ID = Date.now().toString()     //아이디는 유니크하게 현재 시간으로?
    const newTaskObject = {
    ...
    // setTasks({...tasks, ...newTaskObject}); //객체합성 by 스프레드문법
